...
func init() {
	rootCmd.AddCommand(xgllCmd)

	xgllCmd.Flags().StringP("output", "o", "", "output .xgll file (defaults to stdout)")
	_ = viper.BindPFlag("xgll.output", xgllCmd.Flags().Lookup("output"))
}

func runXGLL(cmd *cobra.Command, args []string) error {
	filename := args[0]
	output := viper.GetString("xgll.output")
